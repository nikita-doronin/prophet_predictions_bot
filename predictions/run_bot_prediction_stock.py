...
def predict_prophet(data,inp_prophet_periods):
    # Prophet Calculations:
    m = Prophet()
    m.add_seasonality(name='weekly', period=1, fourier_order=3, prior_scale=0.1)
    m.fit(data)
...
def create_chart(df,dff):
    ...
    fig.update_layout(
        ...
        height=500,
        width=1100,
        title=("AI Prediction for: "+inp_name+", next "+inp_period), # title of the plot
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            forecast = predict_prophet(data=data, inp_prophet_periods=inp_prophet_periods)
            del data
